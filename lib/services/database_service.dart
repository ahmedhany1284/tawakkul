...
class DatabaseService extends GetxController {
  ...
  Future<void> _copyDatabaseFromAssets({required String path}) async {
    ...
    List<int> bytes = data.buffer.asUint8List(); // Convert the ByteData to a list of bytes
  ...
  Future<void> insertData({required String tableName, required Map<String, dynamic> data}) async {
    await initDatabase();

    await _database.insert(tableName, data, conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  Future<List<Map<String, dynamic>>> readData(
      {required String tableName, String? where, List<Object>? whereArgs}) async {
    await initDatabase();
  ...
  Future<int> updateData(
      {required String table, required Map<String, dynamic> values, String? where, List<dynamic>? whereArgs}) async {
    await initDatabase();
