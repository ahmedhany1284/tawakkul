...
class AudioPlayerHandlerImpl extends BaseAudioHandler {
  ...
  Stream<List<IndexedAudioSource>> get _effectiveSequence =>
      rx.Rx.combineLatest3<List<IndexedAudioSource>?, List<int>?, bool, List<IndexedAudioSource>?>(
  ...
  Stream<QueueState> get queueState => rx.Rx.combineLatest3<List<MediaItem>, PlaybackState, List<int>, QueueState>(
      queue,
      playbackState,
...
// Scroll to the page corresponding to the currently playing verse
  void _scrollToPlayingPage(QuranReadingController controller) {
    indexStream = _player.currentIndexStream.listen((currentIndex) async {
      ...
      if (!isWordByWord) {
        controller.highlightVerseAudioHandler(surahNumber: currentSurahID, verseNumber: currentVerseID);
      }
      // Scroll to the page
